...
export default function UniversalPayrollPage() {
  ...
  const saveBatch = async () => {
    ...
    try {
      // 1. Create Run
      const { data: run, error: runError } = await insforge.database.from("universal_payroll_run").insert({
      ...
      const details = parseResult.workers.map((w: any) => ({
        ...
        emp_id: w.emp_id,
        name: w.name,
        iqama_no: w._universal.iqama_no,
        designation: w._universal.designation,
        location: w._universal.location,
        vendor: w._universal.vendor,
        basic_salary: w._universal.basic_salary,
        per_day_rate: w._universal.per_day_rate,
        ot_rate: w._universal.ot_rate,
        working_days: w._universal.working_days,
        weekly_off: w._universal.weekly_off,
        total_payable_days: w._universal.total_payable_days,
        salary: w._universal.salary,
        ot_amount: w._universal.ot_amount,
        tips: w._universal.tips,
        ns_amount: w._universal.ns_amount,
        last_month_adj: w._universal.last_month_adj,
        total_salary: w._universal.total_salary,
        advance: w._universal.advance,
        deduction: w._universal.deduction,
        other_adj: w._universal.other_adj,
        total_payable: w._universal.total_payable,
        payment_status: w._universal.payment_status
      }));

      const { error: detError } = await insforge.database.from("universal_payroll_details").insert(details);
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto animate-in fade-in duration-700">
      ...
      {parseResult ? (
        <div className="space-y-6">
          <LogicCard clientSlug="universal" />
