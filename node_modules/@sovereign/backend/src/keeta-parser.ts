...
export interface KeetaBillingRow {
  courier_id: string;
  courier_name: string;
  partner_id: string;
  partner_name: string;
  billing_cycle: string;
  is_valid: string;       // "valid" | "invalid" | "-"
  reason: string;
  online_days: number;
  daily_online_hours: number;
  peak_hours: number;
  delivered_orders: number;
  order_based_pricing: number;
  distance_pricing: number;
  da_capacity_incentive: number;
  experience_incentive: number;
  dxgy: number;
  subsidy: number;
  activities_rewards: number;
  keeta_deduction: number;
  food_compensation: number;
  other_adjustment: number;
  tips: number;
  tga_deduction: number;
  total_payable_keeta: string;
}

export interface KeetaReferenceRow {
  courier_id: string;
...
export interface KeetaPenaltyRow {
  iqama_no: string;
  name: string;
  project: string;
  city: string;
...
export interface KeetaParseResult {
  billing: KeetaBillingRow[];
  reference: KeetaReferenceRow[];
  penalties: KeetaPenaltyRow[];
  config: {
    spo_basic: number;
    out_basic: number;
    billing_cycle: string;
 ...
 */
export function parseKeetaWorkbook(workbook: XLSX.WorkBook): KeetaParseResult {
  const result: KeetaParseResult = {
    billing: [],
    reference: [],
    penalties: [],
    config: {
  ...
  try {
    ...
    // DETECT SINGLE SHEET CONSOLIDATED FORMAT
    // ═══════════════════════════════════════════════════
    let inputSheet: XLSX.WorkSheet | null = null;
    let rawData: any[][] = [];
    ...
    if (inputSheet) {
      ...
      // ═══════════════════════════════════════════
      if (rawData[0] && rawData[0][0]?.toString().toLowerCase().includes('spo')) {
        result.config.spo_basic = Number(rawData[0][1]) || 2000;
        // Optionally read threshold/rate if needed by engine
        (result.config as any).threshold = Number(rawData[0][4]) || 350;
        (result.config as any).ot_rate = Number(rawData[0][7]) || 5;
      }
      if (rawData[1] && rawData[1][0]?.toString().toLowerCase().includes('out')) {
        result.config.out_basic = Number(rawData[1][1]) || 5500;
        (result.config as any).net_bill = Number(rawData[1][4]) || 200;
        (result.config as any).invalid_rate = Number(rawData[1][7]) || 5;
      }
      ...
      for (let i = 4; i < rawData.length; i++) {
        const row = rawData[i];
        if (!row || !row[idx.id] || String(row[idx.id]).trim() === '' || String(row[idx.id]).toLowerCase().includes('id')) continue;
        ...
        (result as any).manualValues = (result as any).manualValues || {};
        (result as any).manualValues[courierId] = {
    ...
    if (billingSheet) {
      const data: any[][] = XLSX.utils.sheet_to_json(billingSheet, { header: 1, raw: true, defval: '' });

      // Row 0: SPO basic salary, Row 1: OUT basic salary
      if (data[0] && data[0][0] === 'SPO') result.config.spo_basic = Number(data[0][1]) || 2000;
      if (data[1] && data[1][0] === 'OUT') result.config.out_basic = Number(data[1][1]) || 5500;

      for (let i = 4; i < data.length; i++) {
        const row = data[i];
        if (!row || !row[3]) continue; 
        const courierId = String(row[3]).trim();
        if (!courierId || courierId === '' || courierId === 'undefined') continue;
    ...
    if (refSheet) {
      const data: any[][] = XLSX.utils.sheet_to_json(refSheet, { header: 1, raw: true, defval: '' });
    ...
    if (helpSheet) {
      const data: any[][] = XLSX.utils.sheet_to_json(helpSheet, { header: 1, raw: true, defval: '' });
      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        if (!row || !row[0]) continue;
    ...
    const ignoreSheets = [
      'ksa_payable', 'reference', 'Help', 'Invoice', 'Guidelines',
      'Pivot Table 9', 'All Formulas', 'ERP Spec', 'Incentive Table',
      'Summary_KSA', 'Invalid Summary_KSA',
    ];
    (result as any).manualValues = (result as any).manualValues || {};
    ...
    workbook.SheetNames.forEach(sheetName => {
      ...
      for (let i = 3; i < data.length; i++) {
        ...
        if (courierId.length > 10 && !isNaN(Number(courierId))) {
          const existing = (result as any).manualValues[courierId] || {};
          ...
          const salaryCitySheet  = isFinite(Number(row[15])) && row[15] !== '' ? Number(row[15]) : (existing.salary_city_sheet ?? null);
          const incentiveCol16   = isFinite(Number(row[16])) && row[16] !== '' ? Number(row[16]) : (existing.incentive_nafouz ?? 0);
          const totalSalaryCitySheet = isFinite(Number(row[18])) && row[18] !== '' ? Number(row[18]) : (existing.total_salary_city_sheet ?? null);

          const netBill  = isFinite(Number(row[17])) && row[17] !== '' ? Number(row[17]) : (existing.net_bill ?? 0);
          const iqama    = isFinite(Number(row[19])) && row[19] !== '' ? Number(row[19]) : (existing.iqama_renewal ?? 0);
          const traffic  = isFinite(Number(row[20])) && row[20] !== '' ? Number(row[20]) : (existing.traffic_violation ?? 0);
          const vehicle  = isFinite(Number(row[21])) && row[21] !== '' ? Number(row[21]) : (existing.vehicle_repairing ?? 0);
          const dl       = isFinite(Number(row[22])) && row[22] !== '' ? Number(row[22]) : (existing.driving_license_cost ?? 0);
          const advance  = isFinite(Number(row[23])) && row[23] !== '' ? Number(row[23]) : (existing.advance_amount ?? 0);
          ...
          const penalty    = penaltyRaw !== null ? penaltyRaw : (existing.internal_penalty ?? null);
