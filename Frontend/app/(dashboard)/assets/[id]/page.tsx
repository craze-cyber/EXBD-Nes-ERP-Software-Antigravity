...
export default function AssetDetailPage({ params }: { params: { id: string } }) {
  ...
  const loadData = async () => {
    const { data: aData } = await insforge.database.from("assets").select("*, workers(name_en)").eq("id", params.id).single();
    if (aData) setAsset(aData);
    
    const { data: moveData } = await insforge.database.from("asset_movements").select("*, workers!from_worker(name_en), workers!to_worker(name_en)").eq("asset_id", params.id).order("movement_date", { ascending: false });
    if (moveData) setMovements(moveData);
    
    const { data: maintData } = await insforge.database.from("asset_maintenance").select("*").eq("asset_id", params.id).order("start_date", { ascending: false });
