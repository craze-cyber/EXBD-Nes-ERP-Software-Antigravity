...
import { UploadCloud, CheckCircle2, Building2, Users } from "lucide-react";
...
import { parseNoonSuperWorkbook, NoonSuperRow } from "@/lib/noon-super-parser";

export default function NoonSupermallPayrollPage() {
  const [clients, setClients] = useState<any[]>([]);
  const [selectedClient, setSelectedClient] = useState<string | null>(null);
  const [payPeriod, setPayPeriod] = useState(new Date().toISOString().slice(0, 7));
  const [salaryRows, setSalaryRows] = useState<NoonSuperRow[]>([]);
  ...
  useEffect(() => {
    const fetchClients = async () => {
      const { data } = await insforge.database.from("clients").select("*").eq("client_code", "NSM_YS").single();
      if (data) {
        setClients([data]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const rows = await parseNoonSuperWorkbook(wb, selectedClient);
      
      setSalaryRows(rows);
      toast.success(`✅ Noon Supermall: ${rows.length} records parsed`);
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto animate-in fade-in duration-700">
       <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tighter flex items-center gap-3">
            <span className="bg-yellow-600 p-2 rounded-xl border border-yellow-500/20 shadow-lg shadow-yellow-600/20">
              <Building2 className="w-8 h-8 text-black" />
            </span>
            NOON <span className="text-yellow-500 font-light italic">SUPERMALL</span>
          </h1>
          <p className="text-zinc-500 mt-1 font-medium">Enterprise logistics delivery processing mapped to KSA Summary</p>
        </div>

        <div className="flex items-center gap-3">
           <button 
                onClick={() => fileInputRef.current?.click()}
                className="bg-yellow-600 text-black px-6 py-2.5 rounded-xl font-black text-sm hover:scale-105 transition-all shadow-lg shadow-yellow-600/20"
            >
       ...
       {salaryRows.length > 0 ? (
          ...
          <LogicCard clientSlug="noon-supermall" />
                            ...
                            <tbody className="divide-y divide-white/5">
                                {salaryRows.map((r, i) => (
                                    <tr key={i} className="hover:bg-white/[0.02]">
                                        ...
                                        <td className="px-5 py-4">
                                            <span className="text-[9px] px-2 py-0.5 rounded-full font-black uppercase bg-yellow-600/10 text-yellow-500">
                                                {r.location || 'Unknown'}
                                            </span>
                                        </td>
           ...
           <div className="bg-[#121214] border border-dashed border-white/10 rounded-[3rem] p-40 text-center flex flex-col items-center justify-center">
               ...
               <h3 className="text-2xl font-black text-white">Noon Supermall Engine</h3>
               <p className="text-zinc-500 mt-2 max-w-sm">Import the salary template to process exact payables securely.</p>
           </div>
       )}
      {manageWorker && (
        <ManageWorkerModal worker={manageWorker} onClose={() => setManageWorker(null)} period={payPeriod} clientSlug="noon-supermall" />
      )}
    </div>
  );
}
