...
import { UploadCloud, CheckCircle2, HardHat, Users } from "lucide-react";
...
import { parseDabdoobManpowerWorkbook, DabdoobManpowerRow } from "@/lib/dabdoob-manpower-parser";

export default function DabdoobManpowerPayrollPage() {
  const [clients, setClients] = useState<any[]>([]);
  const [selectedClient, setSelectedClient] = useState<string | null>(null);
  const [payPeriod, setPayPeriod] = useState(new Date().toISOString().slice(0, 7));
  const [salaryRows, setSalaryRows] = useState<DabdoobManpowerRow[]>([]);
  ...
  useEffect(() => {
    const fetchClients = async () => {
      const { data } = await insforge.database.from("clients").select("*").eq("client_code", "DBM_YS").single();
      if (data) {
        setClients([data]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const rows = await parseDabdoobManpowerWorkbook(wb, selectedClient);
      
      setSalaryRows(rows);
      toast.success(`✅ Dabdoob Manpower: ${rows.length} records parsed`);
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto animate-in fade-in duration-700">
       <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tighter flex items-center gap-3">
            <span className="bg-emerald-600 p-2 rounded-xl border border-emerald-500/20 shadow-lg shadow-emerald-600/20">
              <HardHat className="w-8 h-8 text-black" />
            </span>
            DABDOOB <span className="text-emerald-500 font-light italic">MANPOWER</span>
          </h1>
          <p className="text-zinc-500 mt-1 font-medium">Labor & Overtime calculation engine mapped to KSA Summary</p>
        </div>

        <div className="flex items-center gap-3">
           <button 
                onClick={() => fileInputRef.current?.click()}
                className="bg-emerald-600 text-black px-6 py-2.5 rounded-xl font-black text-sm hover:scale-105 transition-all shadow-lg"
            >
       ...
       {salaryRows.length > 0 ? (
          ...
          <LogicCard clientSlug="dabdoob-manpower" />
           ...
           <div className="bg-[#121214] border border-dashed border-white/10 rounded-[3rem] p-40 text-center flex flex-col items-center justify-center">
               ...
               <h3 className="text-2xl font-black text-white">Dabdoob Manpower Engine</h3>
               <p className="text-zinc-500 mt-2 max-w-sm">Import the manpower salary template to process exact labor payables securely.</p>
           </div>
       )}
      {manageWorker && (
        <ManageWorkerModal worker={manageWorker} onClose={() => setManageWorker(null)} period={payPeriod} clientSlug="dabdoob-manpower" />
      )}
    </div>
  );
}
