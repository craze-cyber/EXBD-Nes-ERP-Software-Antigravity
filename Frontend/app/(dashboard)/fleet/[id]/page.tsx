...
export default function VehicleDetailPage({ params }: { params: { id: string } }) {
  ...
  const loadData = async () => {
    const [vRes, fRes, mRes, tRes, cRes] = await Promise.all([
      insforge.database.from("vehicles").select("*, workers(name_en, emp_id, mobile), clients(legal_name)").eq("id", params.id).single(),
      insforge.database.from("fleet_fuel_logs").select("*, workers(name_en)").eq("vehicle_id", params.id).order("log_date", { ascending: false }),
      insforge.database.from("fleet_maintenance").select("*").eq("vehicle_id", params.id).order("service_date", { ascending: false }),
      insforge.database.from("fleet_trips").select("*, workers(name_en)").eq("vehicle_id", params.id).order("trip_date", { ascending: false }),
      insforge.database.from("compliance_documents").select("*").eq("entity_id", params.id),
    ]);
