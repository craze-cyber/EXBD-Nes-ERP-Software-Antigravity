...
let result = "const TRUTH_MAP: Record<string, { basic: number, hours: number, net: number, name: string }> = {\n";
let tGross = 0;
let tNet = 0;
...
for (let r = 0; r < jsonData.length; r++) {
    ...
    let empId = "";
    for (let c = 0; c < 10; c++) {
        if (/^n\d+/i.test(String(row[c]).trim())) empId = String(row[c]).trim().toUpperCase();
    }
    if (empId) {
        let name = String(row[1] || row[2] || "").trim();
        if(name.length < 2) name = String(row[2]).trim();
